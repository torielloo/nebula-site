...
export default class CallRoom extends Actor {
  ...
  async handleConnect(conn) {
    // Toda conexão (participante ou observador) precisa de identidade
    // autenticada — o app é público e a sala expõe presença + sinalização.
    if (conn.identity?.type !== 'authenticated' || typeof conn.identity.userId !== 'string') {
      conn.reject(4003, 'authentication required');
      return;
    }
  ...
  async handleMessage(conn, msg) {
    ...
    if (msg.type === 'profile') {
      ...
      // userId deriva SEMPRE da identidade autenticada — nunca do cliente.
