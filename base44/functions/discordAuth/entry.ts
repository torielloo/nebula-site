...
function validRedirect(value, requestOrigin = '') {
  ...
  try {
    ...
    const anyBase44PublishedHost = /^[a-z0-9-]+\.base44\.app$/i.test(host);
    const officialPreview = /^(?:preview|preview-sandbox)--[a-z0-9-]+\.base44\.app$/i.test(host);
    ...
    return url.protocol === 'https:'
      && (knownHost || anyBase44PublishedHost || officialPreview || sameRequestOrigin)
...
export default async function(req) {
  try {
    ...
    const sealSecret = secrets.get('DISCORD_ACCOUNT_SEAL_SECRET') || clientSecret;
    ...
    if (action === 'exchange') {
      ...
      if (linked.length > 0) {
        ...
        try {
          ...
          return Response.json({ status: 'exists', link_token: linkToken, discord_id: profile.discord_id, discord_username: profile.discord_username, discord_handle: profile.discord_handle, discord_avatar_url: profile.discord_avatar_url, email });
        }
      }
      ...
      if (profile.email) {
        ...
        if (userExists) {
          ...
          return Response.json({ status: 'exists', link_token: linkToken, discord_id: profile.discord_id, discord_username: profile.discord_username, discord_handle: profile.discord_handle, discord_avatar_url: profile.discord_avatar_url, email: profile.email });
        }
        ...
        return Response.json({ status: 'register', link_token: linkToken, discord_id: profile.discord_id, discord_username: profile.discord_username, discord_handle: profile.discord_handle, discord_avatar_url: profile.discord_avatar_url, email: profile.email });
      }

      return Response.json({ status: 'no_email', link_token: linkToken, discord_id: profile.discord_id, discord_username: profile.discord_username, discord_handle: profile.discord_handle, discord_avatar_url: profile.discord_avatar_url });
    }
    ...
    if (action === 'link') {
      ...
      const record = {
        ...
        avatar_url: typeof body.discord_avatar_url === 'string' ? body.discord_avatar_url.slice(0, 300) : '',
      };
