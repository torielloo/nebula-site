  ...
  try {
    ...
    for (const c of conversations) {
      ...
      for (const m of metas) {
        ...
        contacts.push({
          id: m.id,
          name: String(m.name || 'Usuário'),
          avatar: String(m.avatar || ''),
          banner: String(m.banner || ''),
          frame: String(m.frame || ''),
          custom_frame_url: String(m.custom_frame_url || ''),
        });
      }
    }
    ...
    for (const u of users) {
      ...
      contacts.push({
        id: u.id,
        name,
        avatar: String(u?.profile?.avatar_url || u?.profile?.discord_avatar_url || ''),
        banner: String(u?.profile?.banner_url || u?.profile?.discord_banner_url || ''),
        frame: String(u?.profile?.frame || ''),
        custom_frame_url: String(u?.profile?.custom_frame_url || ''),
      });
    }
