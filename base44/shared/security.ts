...
const TRUSTED_ORIGINS = new Set([
  ...
  'https://nebula-central.base44.app',
...
function isTrustedOrigin(origin: string) {
  ...
  // Aceita somente previews ligados a este projeto/slugs conhecidos.
  // Não confie genericamente em qualquer *.base44.app: outro app Base44 não
  // deve virar origem confiável só por compartilhar o provedor de hospedagem.
  return /^https:\/\/(?:preview|preview-sandbox)--(?:nebula-os-core-pingu|nebula-os-site-1|nebula-central|6aa87196309472108abb65fb)\.base44\.app$/i.test(origin);
}
...
function securityLogSecret() {
  return secrets.get('DISCORD_CLIENT_SECRET') || EPHEMERAL_SECURITY_SECRET;
}
...
const PROMPT_INJECTION_STRONG = [
  ...
  { re: /\b(eu\s+sou|sou|i\s+am|i'm|aqui\s+[eé]|quem\s+fala\s+[eé])\s+(o\s+|a\s+)?(pingutn|pingu\s*tn|nebulaticos|24k\s*murilo|24kmurilo)\b.{0,180}\b(fa[cç]a|execute|libere|autorize|mostre|revele|mude|altere|apague|banir|bana|desbana|promova|conceda|me\s+d[eê]|me\s+mostre|ignore|desconsidere)\b/i, code: 'privileged_role_impersonation' },
...
export async function guardRequest(req: Request, base44: any, options: {
  ...
  // Estar autenticado não transforma uma origem HTTPS arbitrária em confiável.
  // Requisições server-to-server/CLI sem Origin continuam permitidas por
  // isTrustedOrigin(''), enquanto navegadores precisam vir de origem conhecida.
  const originAllowed = isTrustedOrigin(origin) || explicitExtraOrigin;
  ...
  if (req.method === 'POST') {
    ...
    const acceptedMultipart = allowMultipart && contentType.includes('multipart/form-data');
    if (contentType && !contentType.includes('application/json') && !acceptedMultipart) {
      await writeEvent(base44, {
