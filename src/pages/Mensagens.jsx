...
export default function Mensagens() {
  ...
  const load = async () => {
    try {
      const res = await base44.functions.invoke("listDirectConversations", {});
      ...
      setSelected((current) => {
        ...
        if (!canUseCore && (current.participants || []).includes("core-os")) return null;
        return visible.find((conv) => conv.id === current.id) || current;
      });
      setListError("");
    } catch {
            ...
            visibleConversations.map((conv) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "relative mb-1.5 flex w-full cursor-pointer items-center gap-3 overflow-hidden rounded-2xl border p-3 text-left transition-all duration-200",
                    active
                ...
                >
                  {partner.banner ? (
                    <>
                      <img src={partner.banner} alt="" aria-hidden="true" className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-30" />
                      <span className="pointer-events-none absolute inset-0 bg-gradient-to-r from-background/92 via-background/78 to-background/55" />
                    </>
                  ) : null}
                  <ProfileAvatar
                    name={partner.name}
                    ...
                    frame={partner.frame}
                    customFrameUrl={partner.custom_frame_url}
