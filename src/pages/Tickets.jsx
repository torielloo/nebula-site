    ...
    const loadMentions = () => base44.functions.invoke("ticketOps", { action: "list_my_ticket_mentions" })
      .then((res) => {
        ...
        if (selected?.id && ids.has(selected.id)) {
          ...
          base44.functions.invoke("ticketOps", { action: "mark_ticket_mentions_seen", ticket_id: selected.id }).catch(() => {});
        }
      ...
      <div className="space-y-6">
        ...
        {tickets && tickets.length > 0 && (
          <div className="grid gap-4 lg:grid-cols-[340px,1fr]">
            <div className={cn("scrollbar-thin max-h-[calc(100dvh-15rem)] space-y-2 overflow-y-auto sm:max-h-[70vh] lg:max-h-none", selected && "hidden sm:block")}>
              {filtered.length === 0 && (
