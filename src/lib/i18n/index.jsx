...
import en from "./en";
import es from "./es";
import { nebulaExtras } from "./nebulaExtras";
 ...
 */
const DICTS = {
  pt: { ...pt, ...nebulaExtras.pt },
  en: { ...en, ...nebulaExtras.en },
  es: { ...es, ...nebulaExtras.es },
};
...
function detectLang() {
  ...
  // Português do Brasil é o idioma padrão do Nébula. A preferência do
  // usuário continua sendo respeitada quando ele troca manualmente.
  return "pt";
}
...
export function LangProvider({ children }) {
  ...
  const value = useMemo(() => {
    ...
    const t = (key, vars) => {
      const raw = dict[key] !== undefined ? dict[key] : DICTS.pt[key] !== undefined ? DICTS.pt[key] : key;
      if (!vars) return raw;
...
export function useI18n() {
  ...
  if (!ctx) {
    const t = (key, vars) => {
      const raw = DICTS.pt[key] !== undefined ? DICTS.pt[key] : key;
      if (!vars) return raw;
