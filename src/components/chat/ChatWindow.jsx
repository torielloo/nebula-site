  ...
  return (
    <div className="relative flex h-full flex-col">
      <header className="flex items-center gap-3 border-b border-border/60 px-4 py-3">
        ...
        {partner.id ? (
          <UserQuickCard userId={partner.id}>
            <button type="button" className="flex min-w-0 items-center gap-3 rounded-xl text-left transition-opacity hover:opacity-80">
              <ProfileAvatar name={partner.name} avatar={partner.avatar} size="sm" />
        ...
        ) : <><ProfileAvatar name={partner.name} avatar={partner.avatar} size="sm" /><div className="min-w-0"><p className="truncate text-sm font-bold">{partner.name}</p><p className="text-[11px] text-muted-foreground">{t("dm.private")}</p></div></>}
