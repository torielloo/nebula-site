...
export default function MessageAttachments({ attachments, ticketId, conversationId, resolvedUrls = {}, deferSigning = false }) {
  ...
  const signOne = async (a, { force = false } = {}) => {
    ...
    if (!force && typeof a.signed_url === "string" && /^https?:\/\//i.test(a.signed_url)) return a.signed_url;
  ...
  useEffect(() => {
    ...
    let alive = true;
    let retryTimer = null;
    const retryDelays = [500, 1200, 2500, 5000];

    const resolveUrls = async (attempt = 0) => {
      const pairs = await Promise.all(list.map(async (a) => {
        try {
          const signedUrl = await signOne(a, { force: attempt > 0 });
      ...
      const unresolvedPrivate = list.some((a) => !isPublic(a.url) && !next[a.url] && !resolvedUrls[a.url]);
      if (unresolvedPrivate && attempt < retryDelays.length) {
        retryTimer = window.setTimeout(() => resolveUrls(attempt + 1), retryDelays[attempt]);
      }
    };
  ...
  const urlOf = (a) => signed[a.url] || resolvedUrls[a.url] || (typeof a.signed_url === "string" && /^https?:\/\//i.test(a.signed_url) ? a.signed_url : null) || (isPublic(a.url) ? a.url : null);
