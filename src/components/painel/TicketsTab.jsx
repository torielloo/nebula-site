...
export default function TicketsTab({ tickets, user, onChanged, initialTicketId = null, onConsumeInitialTicket }) {
  ...
  useEffect(() => {
    ...
    const loadMentions = () => base44.functions.invoke("ticketOps", { action: "list_my_ticket_mentions" })
      .then((res) => {
        if (cancelled) return;
        const ids = new Set(res.data?.ticket_ids || []);
        if (current?.id && ids.has(current.id)) {
          ids.delete(current.id);
          base44.functions.invoke("ticketOps", { action: "mark_ticket_mentions_seen", ticket_id: current.id }).catch(() => {});
        }
  ...
  return (
    <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(260px,340px),minmax(0,1fr)]">
      <div className={cn("min-w-0 space-y-3 xl:sticky xl:top-20 xl:self-start", current && "hidden sm:block")}>
        ...
        <div
          className="scrollbar-thin max-h-[calc(100dvh-15rem)] overflow-y-auto pr-1 sm:max-h-[56dvh] xl:max-h-[calc(100dvh-210px)]"
          onDragOver={(event) => {
            ...
            return (
              <React.Fragment key={tk.id}>
                ...
                <div
                  ...
                  onClick={() => {
                    if (!listDraggedRef.current && !isDragging) setSelectedId(tk.id);
                  }}
                  role="button"
                  tabIndex={0}
                  onKeyDown={(event) => {
          ...
          <div className="flex flex-col">
            ...
            <div>
              <TicketThread
                ...
                onMentionSeen={(ticketId) => setMentionedTicketIds((currentIds) => {
                  const next = new Set(currentIds);
