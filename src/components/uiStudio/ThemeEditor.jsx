  ...
  return (
    <div className="space-y-6">
        ...
        {UI_PRESETS.map((p) => (
          ...
          >
            ...
            <span className="min-w-0">
              ...
              <span className="block text-[11px] text-muted-foreground">{t(`uistudio.preset.${p.id === "cyber-red" ? "cyber" : p.id === "minimal-ios" ? "minimal" : "gamer"}`)}</span>
            </span>
          </button>
        ))}
      </section>
      ...
      <section className="space-y-3 border-t border-border/40 pt-4">
        ...
        <Button variant="destructive" size="sm" className="w-full" onClick={ui.resetAll}>
          <RefreshCw className="h-3.5 w-3.5" /> {t("uistudio.restore_all")}
        </Button>
      </section>
    </div>
  );
