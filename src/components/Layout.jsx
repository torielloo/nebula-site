...
function LayoutInner() {
  ...
  const rawAccentValue = profile.accent || siteConfig.theme?.accent || null;
  const gamerRgbActive = ui.config.themePreset === "gamer";
  const legacyRedDefault =
    !profile.accent_source &&
  ...
  const accentStyle = effectiveAccent
    ? { "--primary": effectiveAccent, "--ring": effectiveAccent, "--primary-foreground": primaryForeground }
    : undefined;
  ...
  return (
    <div style={accentStyle} className={cn("min-h-screen overflow-x-clip bg-background font-body text-foreground", gamerRgbActive && "ui-theme-gamer-rgb", ui.editMode && "ui-editing")}>
      ...
      {(() => {
        ...
        const siteBg = pageBg || profile.background_url || siteConfig.theme?.background_url;
        return siteBg ? (
          <>
      ...
      <motion.header
        ...
        animate={{ y: navHidden ? "-130%" : 0, opacity: navHidden ? 0 : 1 }}
        ...
        <div className="relative mx-auto flex h-12 w-full max-w-[1600px] items-center gap-1.5 px-0 sm:h-14 sm:gap-3 sm:px-2 md:px-6">
          ...
          <div className="ml-auto flex min-w-0 items-center gap-1 sm:gap-2">
            ...
            <Link to="/perfil" className="flex shrink-0 items-center gap-2 rounded-full bg-card px-1 py-1.5 transition-colors sm:px-2">
              <ProfileAvatar name={name} avatar={profile.avatar_url} size="sm" status={profile.status} frame={profile.frame} />
      ...
      <footer className="border-t border-white/[0.06] px-4 py-5 text-center text-[11px] font-medium text-muted-foreground md:px-6">
        Todos os direitos reservados para Nebulaticos PinguTn e 24kMurilo
      </footer>
